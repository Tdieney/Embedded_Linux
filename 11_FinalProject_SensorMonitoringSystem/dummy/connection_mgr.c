...
void *connection_mgr_run(void *arg) {
    ...
    while (1) {
        ...
        // Read sensor data
        while (read(client_fd, &data, sizeof(sensor_data_t)) > 0) {
            if (data.sensor_id <= 0) {
                char msg[64];
            ...
            char msg[64];
            snprintf(msg, sizeof(msg), "A sensor node with %d has opened a new connection", data.sensor_id);
            log_event(args->fifo_name, msg);
        ...
        snprintf(msg, sizeof(msg), "The sensor node with %d has closed the connection", data.sensor_id);
        log_event(args->fifo_name, msg);
        close(client_fd);
    }
